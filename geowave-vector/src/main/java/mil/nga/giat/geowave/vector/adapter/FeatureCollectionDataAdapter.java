...
import mil.nga.giat.geowave.store.dimension.SpatialArrayField;
import mil.nga.giat.geowave.store.dimension.SpatialField;
import mil.nga.giat.geowave.store.dimension.TimeArrayField;
import mil.nga.giat.geowave.store.dimension.TimeField;
import mil.nga.giat.geowave.store.index.BasicIndexModel;
...
import mil.nga.giat.geowave.vector.FitToIndexDefaultFeatureCollection;
		...
		ModelConvertingDataAdapter<DefaultFeatureCollection>
{
	...
	private static class SimpleFeatureWrapper
	{
		...
		if (index.getIndexStrategy() instanceof HierarchicalNumericIndexStrategy) {
			...
			while (featItr.hasNext() || (reprocessQueue.size() > 0)) {
				...
				if (featItr.hasNext()) {
					...
					final SimpleFeatureWrapper featWrapper = reprocessQueue.remove(0);
